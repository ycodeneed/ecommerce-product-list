...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    setTimeout(() => {STORE.loader.remove();STORE.showSkeletonLoader();}, 700);
    setTimeout(() => {STORE.renderProducts(Object.values(products));}, 1700);
});
...
var STORE = {
    init: function () {
        ...
        encodeLib.outerClickFunctions.push(STORE.sitebarOuterClickFunc);

        var categoryFilter = document.getElementById('categoryFilter');
        var sortOptions = document.getElementById('sortOptions');
        categoryFilter.addEventListener('change', STORE.filterAndSortProducts);
        sortOptions.addEventListener('change', STORE.filterAndSortProducts);
        
        document.getElementById('nonePrice').addEventListener('change', STORE.filterAndSortProducts);
        document.getElementById('priceUnder250').addEventListener('change', STORE.filterAndSortProducts);
        document.getElementById('price250to500').addEventListener('change', STORE.filterAndSortProducts);
        document.getElementById('priceAbove500').addEventListener('change', STORE.filterAndSortProducts);
        $(".side-bar-button").click(function() {encodeLib.toggleEvent(this, STORE.openFilterBar, STORE.closeFilterBar);});
    ...
    filterAndSortProducts: function() {
        ...
        STORE.renderProducts(filteredProducts);
        
    },
    sitebarOuterClickFunc: function(e) {
        let thisElement = $(".filter-bar");
        let thisElementExtra = $(".side-bar-button");
        if(!thisElement.is(e.target) && thisElement.has(e.target).length === 0 && !thisElementExtra.is(e.target) && thisElementExtra.has(e.target).length === 0) {
            $(".side-bar-button").attr("on", "false");
            STORE.closeFilterBar();
        }
    },
                                ...
                                <div class="product-inner" title="${appInfoObj.name}">
                                    ...
                                    <div>
                                        ...
                                        <div class="product-addCart">
                                            <button onclick="STORE.rippleAction(this, event);" class="product-addCart-button product-Cart-button">
